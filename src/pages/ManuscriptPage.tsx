...
export default function ManuscriptPage() {
  ...
  async function importChapters() {
    setImporting(true);
    const files = await api.importPickFiles([
      { name: "Markdown / Text", extensions: ["md", "txt"] },
    ]);
    if (files.length > 0) {
      await api.importChapters(files);
      loadChapters();
    }
    setImporting(false);
  }
  ...
  return (
    <div style={{ background: "var(--bg)", color: "var(--text)", minHeight: "100vh" }}>
      <AppNav onToggleClaude={() => setClaudeOpen((o) => !o)} claudeOpen={claudeOpen} />
      ...
      <div className="flex h-[calc(100vh-49px)]">
        <div className="flex-1 overflow-y-auto">
          <nav style={{ borderBottom: "1px solid var(--border)" }} className="flex items-center justify-between px-6 py-3">
            ...
            <button onClick={importChapters} disabled={importing}
